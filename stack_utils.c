...
/*   Updated: 2026/03/07 13:14:27 by cescobio         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_add_node_back(t_stack *stack, int value)
{
    ...
    else
    {
        ...
        new->prev = last; // lo conectamos con el nodo anterior (listas doblemente enlazadas)
    }
...
void    ft_assign_index(t_stack *a)
{
    ...
    while (current)
    {
        ...
        while (compare)
        {
            if (compare->value < current->value) // incrementamos el valor por cada numero que sea menor que el actual
                index++;
...
double  ft_compute_disorder(t_stack *a)
{
    ...
    if (total_pairs == 0)
        return (0);
    return ((double)mistakes / total_pairs); //devolvemos porcentaje de pares mal ordenados
}